...
const METRIC_COLUMNS = {
  ...
  "calorie_supply_per_person(kcal/person/day)": {
    label: "Daily Calorie Supply (kcal)",
    format: (val) => `${val.toFixed(0)} kcal`,
  },
  "stunting_rate(%)": {
...
const COLORS = {
  ...
  PRIMARY_BG: "hsla(180, 92%, 51%, 0.08)",
...
async function loadData() {
  try {
    ...
    dpedaData = parseCSV(text).filter(
      (d) => d.Code && d.Code.length === 3 && parseInt(d.Year) >= 1990
    );
...
function populateCountryDropdowns() {
  ...
  countrySelect.value = countries.includes("India")
    ? "India"
    : countries[0] || "";
  countryCompareSelect.value = countries.includes("China")
    ? "China"
...
function getCountryData(countryName, metricColumn, color, bgColor) {
  const filteredData = dpedaData
    .filter(
      (d) =>
        d.Entity === countryName &&
        d[metricColumn] &&
        !isNaN(parseFloat(d[metricColumn]))
    )
    .map((d) => ({
      year: parseInt(d.Year),
      value: parseFloat(d[metricColumn]),
    }))
...
function initChart() {
  ...
  const dataset1 = getCountryData(
    country1,
    metric,
    COLORS.PRIMARY,
    COLORS.PRIMARY_BG
  );
  const dataset2 = getCountryData(
    country2,
    metric,
    COLORS.COMPARE,
    COLORS.COMPARE_BG
  ...
  const allYears = [...new Set([...dataset1.labels, ...dataset2.labels])].sort(
    (a, b) => a - b
  );
  ...
  myChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: `${METRIC_COLUMNS[metric].label} Comparison: ${country1} vs ${country2}`,
          color: "#FFFFFF",
        }, // White title
        ...
        tooltip: {
          callbacks: {
            label: (c) =>
              `${c.dataset.label}: ${METRIC_COLUMNS[metric].format(
                c.parsed.y
              )}`,
          },
        },
      ...
      scales: {
        x: {
          ...
          grid: { color: "#f3f313ff" }, // Darker grid lines
        },
...
document
  .getElementById("country-compare")
